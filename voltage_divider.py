...
def data_dir():
    ...
    files = os.listdir()
    [print(csv + '/' + f)
     for f in files if f.startswith('resistors') and f.endswith('csv')]
    ...
    with open(path) as csv_file:
        ...
        for row in csv_reader:
            if line_count == 0:
                ...
                line_count += 1
        print(f'Loaded {line_count} resistors.')
        return resistorSet
...
def compute(path, vin, v2, tol, r_mw):
    """
    Values from the GUI are used to load the correct resistor set, 
    determine the desired fraction and compute the smallest resistor 
    for each of R1 and R2. Choices are generated and sorted on abs(diff)
    with the smallest first. diff = desired_fract - actual_fract.  
    Returns a namedtuple FinalDesign which the GUI will populate , 
    probably thru a callback method.
    """
    resistorSet = load_resistor_set(path)
    v1 = vin - v2
    min_r1 = v1**2 / (r_mw /1000)
    min_r2 = v2**2 / (r_mw /1000)
    fract = v2 / vin
    design_goals = DesignGoals(vin, v1, v2, r_mw, fract, min_r1, 
                               min_r2, tol)
    choicez = find_choices(design_goals, resistorSet)
    bd = choicez[0]
    """
    VD = namedtuple("VD", "R1 R2 fraction power1_mw power2_mw diff")
    DesignGoals = namedtuple("DesignGoals", "Vin V1 V2 resistor_mw 
                 target_fraction min_r1 min_r2  tolerance") 
    FinalDesign = namedtuple("FinalDesign", "Vin V1 V2 resistor_mw
                   fraction R1_R2 deviance P1_mw P2_mw Max_A2D_V2")
    """
    fv2 = vin * bd.fraction
    fv1 = vin - fv2
    a2d = int(fv2 / 5 * 1023)
    fd = FinalDesign(vin, fv1, fv2, r_mw, bd.fraction, bd.R1, bd.R2,
                     bd.fraction - fract , bd.power1_mw, bd.power2_mw,
                     a2d)
    return fd
...
def main(path):
    ...
    Prints out a list of candidates sorted from smallest to largest 
    diff and selects the one with the smallest deviation from the 
